...
rawComponent =
  mkForwardRefComponent "SheetWrapper" \props ref → React.do
    let
      { header, children, footer, isOpen, onDismiss, containerId } =
        props ∷ { | PropsOptional }
    ...
    let
      ...
      toRender = R.div' </ { ref } />
        ...
        , element window
            { onDismiss, header, children, footer, ref: windowRef, isOpen }
        ]
    pure (renderInPortal toRender)

clickaway ∷ ∀ p. ReactComponent (Record p)
clickaway =
...
window =
  mkForwardRefComponent "SheetWindow" \props ref → React.do
    let
      { header, children, footer, onDismiss, isOpen } =
        props ∷ { | WindowPropsR }
        ...
        [ guard isOpen $ Motion.div
                    ...
                    let
                      ...
                      newY =
                        if (newNaiveValue < maxHeight) && (pi.delta.y < zero) then
                          ...
                          (newNaiveValue < maxHeight) && (pi.delta.y > zero) then
              ...
              , onPanEnd:
                  Motion.onPanEnd \_ pi → do
                    ...
                    let
                      percentPosition =
                        ((yValue - maxHeight) / (windowHeight - maxHeight))
                    let
                      target =
