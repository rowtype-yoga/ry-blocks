...
rawComponent =
  mkForwardRefComponent "Read More" \(props@{ children, moreLabel, lessLabel } ∷ Record PropsOptional) ref -> React.do
    ...
    pure
      $ Motion.span
        ...
        , css: Style.container
        , style:
          R.css { display: if expanded then "inline-block" else "inline-flex" }
        , ref
        }
      /> [ Motion.span
        ...
        , guard showLabel $ Motion.span
            ...
            /> [ R.a'
                      ...
                      handler preventDefault do
                        const do
                          (if expanded then props.onLessClicked else props.onMoreClicked) ?|| mempty
                          modifyExpanded not
                    }
                  /> [ if expanded then
