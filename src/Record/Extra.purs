...
class MapRecord (xs ∷ RL.RowList Type) (row ∷ #Type) a b (from ∷ #Type) (to ∷ #Type) | xs -> row a b from to where
...
instance mapRecordCons ∷
  ...
  mapRecordBuilder _ f r = first <<< rest
    where
...
class ZipRecord (rla ∷ RL.RowList Type) (ra ∷ #Type) (rlb ∷ RL.RowList Type) (rb ∷ #Type) (from ∷ #Type) (to ∷ #Type) | rla -> ra from to
  , rlb -> rb from to where
  zipRecordImpl ∷
    RLProxy rla ->
    Record ra ->
    RLProxy rlb ->
...
instance zipRecordNil ∷ ZipRecord RL.Nil trashA RL.Nil trashB () () where
...
instance ordRecordCons ∷
  ...
  compareRecordImpl _ a b = case compare valA valB of
...
class
  Functor m <= SequenceRecord rl row from to m | rl -> row from to m where
...
instance sequenceRecordSingle ∷
  ...
  sequenceRecordImpl _ a = Builder.insert namep <$> valA
    where
    namep = SProxy ∷ SProxy name
  ...
  sequenceRecordImpl _ a = fn <$> valA <*> rest
    where
    namep = SProxy ∷ SProxy name
