...
const takeWhile = f => x => {
  const $2 = Data$dString$dCodePoints.take(Data$dString$dCodePoints.countPrefix(f)(x))(x);
...
const toNonEmptyCodePointArray = x => {
  const $1 = Data$dString$dCodePoints.toCodePointArray(x);
...
const uncons = nes => (
  {
    head: (() => {
      const $1 = Data$dString$dCodePoints.codePointAt(0)(nes);
      if ($1.tag === "Just") { return $1._1; }
      $runtime.fail();
    })(),
    tail: (() => {
      const $1 = Data$dString$dCodeUnits.drop(Data$dString$dCodeUnits.length(Data$dString$dCodePoints.take(1)(nes)))(nes);
...
const dropWhile = f => x => {
  const $2 = Data$dString$dCodePoints.dropWhile(f)(x);
  if ($2 === "") { return Data$dMaybe.Nothing; }
  return Data$dMaybe.$Maybe("Just", $2);
};
const drop = i => nes => {
