...
const unconsButWithTuple = s => {
  const $1 = uncons(s);
  if ($1.tag === "Just") { return Data$dMaybe.$Maybe("Just", Data$dTuple.$Tuple($1._1.head, $1._1.tail)); }
...
const lastIndexOf = p => s => {
  const $2 = Data$dString$dCodeUnits.lastIndexOf(p)(s);
  if ($2.tag === "Just") { return Data$dMaybe.$Maybe("Just", toCodePointArray(Data$dString$dCodeUnits.take($2._1)(s)).length); }
  return Data$dMaybe.Nothing;
};
const indexOf = p => s => {
  const $2 = Data$dString$dCodeUnits.indexOf(p)(s);
  if ($2.tag === "Just") { return Data$dMaybe.$Maybe("Just", toCodePointArray(Data$dString$dCodeUnits.take($2._1)(s)).length); }
  return Data$dMaybe.Nothing;
};
...
const singletonFallback = v => {
  ...
  return fromCharCode(Data$dEuclideanRing.intDiv(v - 65536 | 0)(1024) + 55296 | 0) + fromCharCode(Data$dEuclideanRing.intMod(v - 65536 | 0)(1024) + 56320 | 0);
};
...
const lastIndexOf$p = p => i => s => {
  const $3 = Data$dString$dCodeUnits.lastIndexOf$p(p)(Data$dString$dCodeUnits.length(take(i)(s)))(s);
  if ($3.tag === "Just") { return Data$dMaybe.$Maybe("Just", toCodePointArray(Data$dString$dCodeUnits.take($3._1)(s)).length); }
  return Data$dMaybe.Nothing;
};
const splitAt = i => s => {
  const before = take(i)(s);
  return {before: before, after: Data$dString$dCodeUnits.drop(Data$dString$dCodeUnits.length(before))(s)};
};
...
const indexOf$p = p => i => s => {
  ...
  const $4 = Data$dString$dCodeUnits.indexOf(p)(s$p);
  if ($4.tag === "Just") { return Data$dMaybe.$Maybe("Just", i + toCodePointArray(Data$dString$dCodeUnits.take($4._1)(s$p)).length | 0); }
  return Data$dMaybe.Nothing;
};
...
const enumCodePoint = {
  succ: a => {
    const $1 = a + 1 | 0;
    if ($1 >= 0 && $1 <= 1114111) { return Data$dMaybe.$Maybe("Just", $1); }
    return Data$dMaybe.Nothing;
  },
  pred: a => {
    const $1 = a - 1 | 0;
