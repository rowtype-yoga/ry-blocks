...
const joinWith1 = dictFoldable1 => {
  const $1 = dictFoldable1.Foldable0();
  return sep => xs => $1.foldl(v => v1 => {
...
const stripPrefix = pat => a => {
  const $2 = Data$dString$dCodeUnits.stripPrefix(pat)(a);
  if ($2.tag === "Just") {
    if ($2._1 === "") { return Data$dMaybe.Nothing; }
    return Data$dMaybe.$Maybe("Just", $2._1);
  }
  if ($2.tag === "Nothing") { return Data$dMaybe.Nothing; }
  $runtime.fail();
};
const stripSuffix = pat => a => {
  const $2 = Data$dString$dCodeUnits.stripSuffix(pat)(a);
  if ($2.tag === "Just") {
    if ($2._1 === "") { return Data$dMaybe.Nothing; }
    return Data$dMaybe.$Maybe("Just", $2._1);
  }
...
const trim = v => {
  const $1 = Data$dString$dCommon.trim(v);
