...
const showTime = {
  show: v => "(Time (Hour " + (Data$dShow.showIntImpl(v._1) + ")") + (
    " (Minute " + (Data$dShow.showIntImpl(v._2) + ")") + (" (Second " + (Data$dShow.showIntImpl(v._3) + ")") + (" (Millisecond " + Data$dShow.showIntImpl(v._4) + "))"))
  )
};
...
const millisToTime = v => {
  ...
  const $4 = Data$dMaybe.applyMaybe.apply(Data$dMaybe.applyMaybe.apply(Data$dMaybe.applyMaybe.apply((() => {
    const $4 = Data$dInt.unsafeClamp(Data$dNumber.floor(hours));
    if ($4 >= 0 && $4 <= 23) { return Data$dMaybe.$Maybe("Just", Time($4)); }
    return Data$dMaybe.Nothing;
  })())((() => {
    const $4 = Data$dInt.unsafeClamp(Data$dNumber.floor(minutes));
    if ($4 >= 0 && $4 <= 59) { return Data$dMaybe.$Maybe("Just", $4); }
    return Data$dMaybe.Nothing;
  })()))((() => {
    const $4 = Data$dInt.unsafeClamp(Data$dNumber.floor(seconds));
    if ($4 >= 0 && $4 <= 59) { return Data$dMaybe.$Maybe("Just", $4); }
    return Data$dMaybe.Nothing;
  })()))((() => {
    const $4 = Data$dInt.unsafeClamp(Data$dNumber.floor(v - (hours * 3600000.0 + minutes * 60000.0 + seconds * 1000.0)));
    if ($4 >= 0 && $4 <= 999) { return Data$dMaybe.$Maybe("Just", $4); }
    return Data$dMaybe.Nothing;
  })());
