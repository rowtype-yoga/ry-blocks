...
const monadTransYoneda = {
  lift: dictMonad => {
    ...
    return m => k => $1.map(k)(m);
  }
};
const hoistYoneda = nat => v => x => nat(v(x));
const functorYoneda = {map: f => m => k => m(x => k(f(x)))};
const extendYoneda = dictExtend => {
  const $1 = dictExtend.Functor0();
  return {extend: f => v => k => dictExtend.extend(x => k(f(k$1 => $1.map(k$1)(x))))(v(identity)), Functor0: () => functorYoneda};
};
...
const ord1Yoneda = dictOrd1 => {
  ...
  const eq1Yoneda1 = {eq1: dictEq => eqYoneda($2)(dictEq).eq};
...
const applicativeYoneda = dictApplicative => {
  ...
  return {
    pure: (() => {
      const $3 = Apply0.Functor0();
      return x => {
        const $5 = dictApplicative.pure(x);
        return k => $3.map(k)($5);
      };
    })(),
    Apply0: () => applyYoneda1
