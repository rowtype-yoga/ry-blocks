...
const toInstant = /* #__PURE__ */ (() => {
  ...
  return a => {
    const $2 = $0(a);
    if ($2.tag === "Just") {
      if ($2._1 >= -8639977881600000.0 && $2._1 <= 8639977881599999.0) { return Data$dMaybe.$Maybe("Just", $2._1); }
...
const toDateTime = x => {
  const $1 = toInstant(x);
  if ($1.tag === "Just") { return Data$dMaybe.$Maybe("Just", Data$dDateTime$dInstant.toDateTime($1._1)); }
  return Data$dMaybe.Nothing;
};
const toDateString = dt => dateMethod("toDateString", dt);
const toDate = x => {
  const $1 = toInstant(x);
  if ($1.tag === "Just") { return Data$dMaybe.$Maybe("Just", Data$dDateTime$dInstant.toDateTime($1._1)._1); }
  return Data$dMaybe.Nothing;
};
...
const fromDateTime = v => jsdate({
  ...
  month: Data$dInt.toNumber((() => {
    if (v._1._2.tag === "January") { return 0; }
    if (v._1._2.tag === "February") { return 1; }
    if (v._1._2.tag === "March") { return 2; }
    if (v._1._2.tag === "April") { return 3; }
    if (v._1._2.tag === "May") { return 4; }
    if (v._1._2.tag === "June") { return 5; }
    if (v._1._2.tag === "July") { return 6; }
    if (v._1._2.tag === "August") { return 7; }
    if (v._1._2.tag === "September") { return 8; }
    if (v._1._2.tag === "October") { return 9; }
    if (v._1._2.tag === "November") { return 10; }
    if (v._1._2.tag === "December") { return 11; }
    $runtime.fail();
  })()),
